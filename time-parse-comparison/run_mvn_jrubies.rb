...
subcommand_output = 'maven_timeparse_subcommand_output.txt'

benchmark_output_files = ['maven_timeparse_benchmark_output',
                          subcommand_output]
...
open(subcommand_output, 'w') do |f|
  jrubies.each do |jruby_version, info|
    ...
    compile_modes.each do |compile_mode|
      ...
      f.puts `mvn -P #{mvn_profile} exec:java -Dexec.mainClass=jruby9k.TimeParseBenchmarkDriver -Dexec.args="#{jruby_version} #{compile_mode}"`
    end
  end
end
